(function ($, window) {
  ...
  $.fn.omniva = function (options) {
    ...
    var UI = {
      hook: $(this),
      loader: $(
        '<div className="omniva-loading-overlay" style="display: none;"></div>'
      ),
      terminal_container: $(
        '<div className="omniva-terminal-container" ' +
          (settings.autoHide ? 'style = "display: none;"' : "") +
          "></div>"
      ),
      container: $('<div className="omniva-terminals-list"></div>'),
      show_on_map_btn: $(
        '<button type="button" className="omniva-btn">' +
          settings.translate.show_on_map +
          '  <img src="' +
          settings.path_to_img +
          'sasi.png" title="' +
          settings.translate.show_on_map +
          '">' +
          "</button>"
      ...
      dropdown: $(
        '<div className="omniva-dropdown">' +
          settings.translate.select_terminal +
          "</div>"
      ),
      search: $(
        '<input type="text" placeholder="' +
          settings.translate.search_bar_placeholder +
          '" className="omniva-search-input"/>'
      ),
      list: $("<ul></ul>"),
      showMapBtn: $(
        '<li><a href="#" className="omniva-show-on-map">' +
          settings.translate.show_on_map +
          "</a></li>"
      ),
      showMore: $(
        '<div className="omniva-show-more"><a href="#">' +
          settings.translate.show_more +
          "</a></div>"
      ).hide(),
      innerContainer: $(
        '<div className="omniva-inner-container"></div>'
      ).hide(),
      modal: $(
        '<div className="omniva-modal">' +
          '  <div className="omniva-modal-content">' +
          '    <div className="omniva-modal-header">' +
          '      <span className="omniva-modal-close">&times;</span>' +
          "      <h5>" +
          settings.translate.modal_header +
          "</h5>" +
          "    </div>" +
          '    <div className="omniva-modal-body">' +
          '      <div className="omniva-map-container"></div>' +
          '      <div className="omniva-search-bar">' +
          "        <h4>" +
          settings.translate.search_bar_title +
          "</h4>" +
          '        <div className="omniva-search">' +
          "          <form>" +
          '            <input type="text" placeholder="' +
          settings.translate.search_bar_placeholder +
          '" />' +
          '            <button type="submit" className="omniva-modal-search-btn"></button>' +
          "          </form>" +
          '          <div className="omniva-autocomplete omniva-scrollbar" style="display:none;">' +
          "            <ul></ul>" +
          "          </div>" +
          "        </div>" +
          '        <div className="omniva-back-to-list" style="display:none;">' +
          settings.translate.search_back_to_list +
          "</div>" +
          '        <div className="omniva-found-terminals omniva-scrollbar omniva-scrollbar-style-8">' +
          "          <ul></ul>" +
          "        </div>" +
          "      </div>" +
          "    </div>" +
    ...
    UI.modal
      .find(".omniva-back-to-list")
      .css("background-image", 'url("' + settings.path_to_img + 'back.png")');
    UI.modal
      .find(".omniva-modal-search-btn")
      .css(
        "background-image",
        'url("' + settings.path_to_img + 'search-w.png")'
      );
    ...
    $(document).on("mousedown", function (e) {
      if (
        !UI.container.is(e.target) &&
        UI.container.has(e.target).length === 0 &&
        UI.container.hasClass("open")
      )
        toggleDropdown();
    });
    UI.modal
    ...
    function refreshList(autoselect) {
      ...
      $(settings.terminals).each(function (i, val) {
        ...
        html +=
          ...
          '    <div id="' +
          makeUID(val[3]) +
          '" className="omniva-details" style="display:none;">' +
          "      <small>" +
          val[5] +
          "<br/>" +
          val[6] +
          "</small><br/>" +
          '      <button type="button" className="omniva-select-terminal-btn" data-id="' +
          val[3] +
          '">' +
          settings.translate.select_terminal +
          "</button>" +
      ...
      if (settings.showMap == true) {
        foundTerminalsEl.html(
          '<ul className="omniva-terminals-listing" start="1">' + html + "</ul>"
        );
      }
    }
    ...
    function findPosition(address, autoselect) {
      ...
      $.getJSON(
        "https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/findAddressCandidates?" +
          prepAddress({ singleLine: address }) +
          "&sourceCountry=" +
          settings.country_code +
          "&category=&outFields=Postal&maxLocations=1&forStorage=false&f=pjson",
        function (data) {
    ...
    function suggest(address) {
      ...
      $.getJSON(
        "https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/suggest?" +
          prepAddress({ text: address }) +
          "&f=pjson&sourceCountry=" +
          settings.country_code +
          "&maxSuggestions=1",
        function (data) {
    ...
    function initMap() {
      var mapEl = $('<div className="omniva-map"></div>')[0];
      ...
      var Icon2 = L.Icon.extend({
        options: { iconSize: [32, 32], iconAnchor: [16, 32] },
      });
      ...
      UI.modal.click(function (e) {
        if (
          !autocompleteEl.is(e.target) &&
          autocompleteEl.has(e.target).length === 0
        ) {
          autocompleteEl.hide();
        }
      });
      ...
      $.getJSON(
        ...
        function (data) {
          if (data.suggestions != undefined && data.suggestions.length > 0) {
            ...
            autocompleteUlEl.append(
              "<li>" + settings.translate.place_not_found + "</li>"
            );
          }
          autocompleteEl.show();
        }
    ...
    function listTerminals(locations, id) {
      ...
      if (id) {
        ...
        if (terminal.length > 0) {
          ...
          var html = "";
          if (!previous_list) {
            previous_list = foundTerminalsEl
              .find(".omniva-terminals-listing")
              .detach();
          }
          ...
          for (var i = 0; i < locations.length; i++) {
            if (locations[i][3] == id) {
              html +=
                ...
                '" >' +
                "<div>" +
                '  <a className="omniva-li"><b>' +
                locations[i][0] +
                "</b></a>" +
                '  <div id="' +
                makeUID(locations[i][3]) +
                '" className="omniva-details">' +
                "  <small>" +
                locations[i][5] +
                " <br/>" +
                locations[i][6] +
          ...
          foundTerminalsEl
            ...
            .append(
              $(
                '<ul className="omniva-terminals-listing" start="1">' +
                  html +
                  "</ul>"
              )
            );
    ...
    function updateActiveMarker(id) {
      map.eachLayer(function (layer) {
        if (
          layer.options.terminalId !== undefined &&
          L.DomUtil.hasClass(layer._icon, "active")
        ) {
